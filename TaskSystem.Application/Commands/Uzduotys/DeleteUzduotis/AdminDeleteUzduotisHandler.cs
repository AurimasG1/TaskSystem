...
public class AdminDeleteUzduotisHandler
{
    private readonly IRepository<Uzduotis> _repo;
    private readonly IUzduotisRepository _taskRepo;

    public AdminDeleteUzduotisHandler(IRepository<Uzduotis> repo, IUzduotisRepository taskRepo)
    {
        _repo = repo;
        _taskRepo = taskRepo;
    }

    public async Task Handle(AdminDeleteUzduotisCommand request)
    {
        var task =
